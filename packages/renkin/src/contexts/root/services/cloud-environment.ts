...
const cloudState = (options?: CloudflareOptions) =>
  Effect.gen(function* () {
    const config = cloudCredentials(options);
    const { endpoint, stateAuthToken } = yield* ensureCloudflareState(config);
    return {
      config,
      endpoint,
      stateAuthToken,
...
const accountSubdomain = (config: CloudStateOptions) =>
  Effect.gen(function* () {
    const { subdomain } = yield* createBootstrapClient(config).getAccountSubdomain();
    if (!subdomain)
      return yield* Effect.fail(new Error("Cloudflare account has no workers.dev subdomain."));
    return subdomain;
  });
const tokenClient = (
  config: CloudStateOptions,
  managementState: CloudflareStateRepository | undefined,
      ...
      ? createAccountTokenClient(
          { ...config, apiToken: options.tokenManagementApiToken },
          {
            request: (request, token) =>
              Effect.runPromise(managementState.gateway(stack, environment, token, request)),
          },
        )
      : undefined;
  return accountTokenClient;
};
export const cloudEnvironment = (
  ...
  Effect.gen(function* () {
    const { config, state, endpoint, stateAuthToken } = yield* cloudState(options);
    const managementState = options?.tokenManagementApiToken
      ...
      : undefined;
    const accountTokenClient = tokenClient(config, managementState, options, stack, environment);
    const subdomain = yield* accountSubdomain(config);
    const gateway = {
      request: (request: GatewayRequest, token: string) =>
        Effect.runPromise(state.gateway(stack, environment, token, request)),
    };
...
export const readCloudState = (options?: CloudflareOptions) =>
  Effect.gen(function* () {
    const config = cloudCredentials(options);
    const located = yield* findCloudflareState(config);
    return located
      ? new CloudflareStateRepository({
