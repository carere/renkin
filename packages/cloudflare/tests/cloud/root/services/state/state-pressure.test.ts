...
const pressureScope = () => {
  ...
  return { accountId, apiToken, prefix };
};
...
it.skipIf(process.env.RENKIN_CLOUDFLARE_STATE_PRESSURE_TESTS !== "true")(
  ...
  async () => {
    const { accountId, apiToken, prefix } = pressureScope();
    ...
    try {
      ...
      lease = await Effect.runPromise(repository.acquire(stack, environment));
      ...
      for (let iteration = 0; iteration < 20; iteration++) {
        ...
        const observed = await Effect.runPromise(lease.read());
        expect(observed?.outputs.payload?.value === payload).toBe(true);
        expect(observed?.outputs.iteration?.value).toBe(iteration);
        expect((await Effect.runPromise(repository.inspect(stack, environment))).leaseActive).toBe(
          true,
        );
      }
      await Effect.runPromise(
        lease.write({ version: 1, stack, environment, resources: {}, outputs: {} }),
      );
      await Effect.runPromise(lease.removeEmpty());
      expect(await Effect.runPromise(repository.read(stack, environment))).toBeUndefined();
      expect(await Effect.runPromise(repository.list(stack))).not.toContain(environment);
    } finally {
      try {
        await Effect.runPromise(lease?.release() ?? Effect.void);
      } finally {
