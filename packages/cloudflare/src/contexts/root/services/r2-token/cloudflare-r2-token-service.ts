...
const configuration = async (
  ...
  if (
    ...
    throw new ResourceOperationError("Invalid scoped R2 token properties.");
  ...
  for (const logicalId of properties.buckets) {
    ...
    if (typeof jurisdiction !== "string")
      throw new ResourceOperationError("Invalid R2 jurisdiction.");
  ...
  const permissions = await Effect.runPromise(client.permissions());
  const names = [
...
class CloudflareR2TokenService implements ResourceService {
  ...
  readonly apply: ResourceService["apply"] = async (
    definition,
    allocation,
    previous,
    resources = {},
  ) => {
    const { client, accountId, leaseToken } = this;
    const { properties, buckets, scopes, jurisdiction, groups } = await configuration(
      client,
      accountId,
      definition,
      resources,
    ...
    if (typeof creationReceipt !== "string")
      throw new ResourceOperationError("R2 token receipt identity is missing.");
    ...
    const replayed = previous
      ? undefined
      : await Effect.runPromise(client.replay(desired, leaseToken, creationReceipt));
    const tokens = await Effect.runPromise(client.list());
    let observed = replayed?.id
      ? replayed
      : tokens.find((item) => (previous ? item.id === previous.physicalId : item.name === name));
    ...
    if (!observed) {
      ...
      if (Date.parse(String(properties.expiresAt)) <= Date.now())
        throw new ResourceOperationError("New R2 token expiry must be in the future.");
      const created = await Effect.runPromise(client.create(desired, leaseToken, creationReceipt));
      observed = created;
      value = created.value;
    }
    if (observed.name !== name)
      throw new ResourceOperationError(
        "Account token ownership differs from its recorded identity.",
      );
  ...
  readonly remove: ResourceService["remove"] = async (resource) => {
    const { client, leaseToken } = this;
    const saved = record(resource.outputs);
    const observed = (await Effect.runPromise(client.list())).find(
    ...
    await Effect.runPromise(client.remove(resource.physicalId, leaseToken));
  };
}
