...
The subsequent exact-host certificate inventory confirmed zero matching packs for
final Astro scope `fadc1fd5`, earlier scopes `54773f62` and `d256e2df`, and
TanStack SSR scope `76e5dcb4`. The previously observed shared provider-managed
pack completed deletion without manual deletion by this task. The temporary
read-only token used for the follow-up was revoked. Application domains, Workers,
KV namespaces and environments had already been removed by their exact scopes.
