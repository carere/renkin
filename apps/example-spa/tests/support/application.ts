...
const inspectBrowser = async (browser: Browser, url: string) => {
  ...
  await page.getByRole("heading", { name: "SPA details" }).waitFor();
...
const inspectBuild = async (build: WorkerBuildResult) => {
  ...
  assert.ok(clientCode.some((source) => source.includes("Solid SPA example")));
};

const check = async () => {
  const directory = await mkdtemp(resolve(tmpdir(), "renkin-spa-"));
      ...
      Effect.scoped(
        Effect.gen(function* () {
          const app = yield* development(
            defineStack({ name: "example-spa-test", resources: [resource] }),
            { directory },
          );
  ...
  try {
    await run(declared, async (url) => {
      ...
      await writeFile(detail, original.replace("SPA details", "SPA live reload"));
      await page.getByRole("heading", { name: "SPA live reload" }).waitFor();
    ...
    await run(worker("Site", { ...options, build }), async (url) => {
      ...
      assert.equal((await fetch(new URL("/unknown-client-route", url))).status, 200);
      await page.close();
    });
  } finally {
    await writeFile(detail, original);
